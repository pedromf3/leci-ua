...
void term_pfifo(PriorityFIFO* pfifo)
{
   ...
   pthread_mutex_destroy(&pfifo->access); // ---
...
void insert_pfifo(PriorityFIFO* pfifo, int id, int priority)
{
   ...
   {
      pthread_cond_wait(&pfifo->not_full, &pfifo->access); // ---
   }
   ...
   if (!pfifo->is_closed)
   {
      ...
      pthread_cond_signal(&pfifo->not_empty); // ---
   }
...
int retrieve_pfifo(PriorityFIFO* pfifo)
{
   ...
   {
      pthread_cond_wait(&pfifo->not_empty, &pfifo->access); // ---
   }
   int result = -1;
   if (!pfifo->is_closed)
   {

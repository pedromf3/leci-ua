...
void init_simulation(int np)
{
   ...
   hd->num_patients = np;
   for (int i = 0; i < np; i++){
      pthread_mutex_init(&hd->all_patients[i].mutexDone, NULL);   // ---
      pthread_cond_init(&hd->all_patients[i].condDone, NULL);  // ---
   }
...
void term_simulation(int np) {
   ...
   for( int i = 0; i < np; i++){
      pthread_mutex_destroy(&hd->all_patients[i].mutexDone); // ---
      pthread_cond_destroy(&hd->all_patients[i].condDone);  // ---
   }
   free(hd);
   hd = NULL;
}
...
int doctor_iteration(int id) // return value can be used to request termination
{
   ...
   pthread_mutex_lock(&hd->all_patients[patient].mutexDone);  // ---
   hd->all_patients[patient].done = 1;
   pthread_cond_signal(&hd->all_patients[patient].condDone); // ---
   pthread_mutex_unlock(&hd->all_patients[patient].mutexDone);    // ---
   return 0;
}
...
void patient_wait_end_of_consultation(int id)
{
   ...
   pthread_mutex_lock(&hd->all_patients[id].mutexDone); // ---
   while(&hd->all_patients[id].done == 0){
      pthread_cond_wait(&hd->all_patients[id].condDone, &hd->all_patients[id].mutexDone);
   }
   pthread_mutex_unlock(&hd->all_patients[id].mutexDone); // ---
...
void* nurse_life(void* arg){ // ----
   int id = (intptr_t)arg;
   while (!is_closed_pfifo(&hd->triage_queue)){
      if (nurse_iteration(id)) break;
   }
   return NULL;
}
...
void* doctor_life(void* arg){ // ----
   int id = (intptr_t)arg;
   while(!is_closed_pfifo(&hd->doctor_queue)){
      if (doctor_iteration(id)) break;
   } 
   return NULL;
}
...
int main(int argc, char *argv[])
{
   ...
   pthread_t nurse_threads[MAX_NURSES]; //--

   for(int i = 0; i < nnurses; i++){
      pthread_create(&nurse_threads[i], NULL, nurse_life, (void*)(intptr_t) i); // ---
   }
   printf("\n");

   pthread_t doctor_threads[MAX_DOCTORS]; //--
   for (int i = 0; i< ndoctors; i++){
      pthread_create(&doctor_threads[i], NULL, doctor_life, (void*)(intptr_t) i); // ---
   }
   printf("\n");
   ...
   {
      pthread_create(&patient_threads[i], NULL, patient_life, (void*)(intptr_t)i); // ---
